...
void AlertListWidget::createUI()
{
    ...
    alertsTable->elementAt(row, 1)->setColumnSpan(4);
    ...
    new Wt::WText(Wt::WString::tr("Alert.alert-list.alert-actions"), alertsTable->elementAt(row, 4));
    ...
    try
    {
        ...
        if (tempUser)
        {
            ...
            Wt::Dbo::collection<boost::tuple<Wt::Dbo::ptr<Alert>,Wt::Dbo::ptr<AlertCriteria>,Wt::Dbo::ptr<AlertValue> > > listTuples = resQuery.resultList();
            for (Wt::Dbo::collection<boost::tuple<Wt::Dbo::ptr<Alert>,Wt::Dbo::ptr<AlertCriteria>,Wt::Dbo::ptr<AlertValue> > >::const_iterator i = listTuples.begin(); i != listTuples.end(); ++i) 
            {
                row++;
...
//                alertsTable->elementAt(row, 4)->setPadding(10);
                ...
                Wt::WPushButton *delButton = new Wt::WPushButton(tr("Alert.alert-list.delete-alert"), alertsTable->elementAt(row, 4));
